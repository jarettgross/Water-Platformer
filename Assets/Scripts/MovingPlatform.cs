	...
	**/
    ...
    IEnumerator handlePlatformMove() {
        ...
        while (!Mathf.Approximately(Mathf.Sqrt(Mathf.Pow(transform.position.x - startPos.x, 2) + Mathf.Pow(transform.position.y - startPos.y, 2)), 0)) { //wait while platform has not moved predefined distance
			if ((horizontalSpeed >= 0) && (verticalSpeed >= 0) && transform.position.x <= startPos.x && transform.position.y <= startPos.y) {
				break;
			} else if ((horizontalSpeed <= 0) && (verticalSpeed >= 0) && transform.position.x >= startPos.x && transform.position.y <= startPos.y) {
				break;
			} else if ((horizontalSpeed >= 0) && (verticalSpeed <= 0) && transform.position.x <= startPos.x && transform.position.y >= startPos.y) {
				break;
			} else if ((horizontalSpeed <= 0) && (verticalSpeed <= 0) && transform.position.x >= startPos.x && transform.position.y >= startPos.y) {
