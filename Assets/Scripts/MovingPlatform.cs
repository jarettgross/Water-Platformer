...
public class MovingPlatform : MonoBehaviour
{
	...
	**/
    ...
    IEnumerator handlePlatformMove()
    {
        ...
        while (Mathf.Sqrt(Mathf.Pow(transform.position.x - startPos.x, 2) + Mathf.Pow(transform.position.y - startPos.y, 2)) < distance)
        { //wait while platform has not moved predefined distance
            yield return null;
        }
        ...
        while (!Mathf.Approximately(Mathf.Sqrt(Mathf.Pow(transform.position.x - startPos.x, 2) + Mathf.Pow(transform.position.y - startPos.y, 2)), 0))
        { //wait while platform has not moved predefined distance
